...
def extract_embeddings(model, dataloader, device="cuda", mode="mean", test=False):
    """
    ...
    if test:
        ...
        with torch.no_grad():
            for imgs, labels, _ in dataloader:
                imgs = imgs.to(device)
                emb  = model.get_embeddings(imgs)          # [batch, 64]
                all_embeddings.append(emb.cpu().numpy())
                all_labels.append(labels.numpy())

        embeddings = np.concatenate(all_embeddings, axis=0)  # [N, 64]
        labels     = np.concatenate(all_labels,     axis=0)  # [N]
      
        # Lista de tuplas (array_64d, label) — una por tumor
        return [(embeddings[i], int(labels[i])) for i in range(len(embeddings))]
